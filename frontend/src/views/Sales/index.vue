  ...
  <div class="sales">
    ...
    <el-dialog v-model="detailVisible" title="销售订单详情" width="800px">
      <el-descriptions :column="2" border>
        ...
        <el-descriptions-item label="总金额">¥{{ selectedSale?.total_amount.toFixed(2) }}</el-descriptions-item>
...
const getSales = async () => {
  ...
  try {
    ...
    let url = `/api/sales/?skip=${(currentPage.value - 1) * pageSize.value}&limit=${pageSize.value}`
    ...
    if (dateRange.value && dateRange.value.length === 2) {
      url += `&start_date=${dateRange.value[0].toISOString().split('T')[0]}&end_date=${dateRange.value[1].toISOString().split('T')[0]}`
    }
